    ...
    container.innerHTML = posts.length > 0 ? posts.map(p => `
        <div class="bg-white dark:bg-neutral-900 rounded-[32px] p-5 border border-gray-200 dark:border-neutral-800 shadow-sm mb-5 transition-colors">
            <div class="flex items-center gap-3 mb-3">
                <img src="${p.users.profile_img_url || 'https://via.placeholder.com/150'}" class="w-10 h-10 rounded-full border border-gray-200 dark:border-neutral-700 shadow-sm object-cover">
                <div class="flex-1">
                    <h4 class="font-bold text-[14px]">${p.users.full_name}</h4>
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Just now</p>
                </div>
            </div>
            <p class="text-[14px] leading-relaxed mb-4 px-1">
                ${p.content}
            </p>
...
async function loadDiscoverStudents() {
    ...
    container.innerHTML = users.map((u, i) => `
        <div class="bg-white dark:bg-neutral-900 rounded-[24px] p-4 border border-gray-200 dark:border-neutral-800 shadow-sm flex items-center gap-4 cursor-pointer hover:border-primary transition-colors" onclick="viewStudentProfile(${i})">
            <img src="${u.profile_img_url || 'https://via.placeholder.com/150'}" class="w-12 h-12 rounded-full object-cover">
            <div class="flex-1">
                <h4 class="font-bold text-[14px]">${u.full_name} ${u.is_private ? '🔒' : ''}</h4>
                <p class="text-[12px] text-gray-500">${u.course || 'Student'} • ${u.connection_count || 0} Conns</p>
            </div>
        </div>
    `).join('');
}
...
// ==========================================
// 7. NOTIFICATIONS SYSTEM (MARK READ)
// ==========================================
window.markNotifRead = async function(notifId) {
    try {
        await sb.from('notifications').update({ is_read: true }).eq('id', notifId);
        loadNotifications(); // Reload to refresh badge and styles
    } catch (err) {
...
// Side-panel Notification Slider
